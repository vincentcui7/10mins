...
def profile(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            p = UserProfile(name=name, sex=sex, password=password, avatar=avatar, belong_to=u)
            p.save()
...
def collection(request):
    ...
    #
    # collect = Ticket.objects.filter(voter_id=user_id , choice='like')
    # collection = Article.objects.filter(
    #     article_tickets.voter_id=user_id,
    #     article_tickets.choice='like'
    # )

    context = {}
    user = request.user
    tickets = user.user_tickets.all()
    like_tickets = tickets.filter(choice='like')

    article =[]
    for ticket in like_tickets:
        article.append(ticket.article)

    # article = Article.objects.all()
    # print('b'*30)
    # print(article)
    # print('b'*30)
    # article_list = Article.objects.filter(title=article)
    # print('b'*30)
    # print(article_list)
    # print('b'*30)

    page_robot = Paginator(article, 3)
    page_num = request.GET.get('page')
    try:
